    ...
    <div className="min-h-screen bg-gradient-to-br from-[#0d0e1b] via-[#1a1025] to-[#0f162e] flex items-center justify-center p-6 overflow-hidden relative">
      ...
      <div className="relative z-10 text-center max-w-2xl w-full">
        ...
        {!isOpened ? (
          ...
          <div 
  className={`
    relative bg-gradient-to-br from-[#1c1438] to-[#2c1e4e] p-4 md:p-12 rounded-3xl 
    border-4 border-[#c9a028]/70 shadow-2xl shadow-purple-950/60
    animate-reveal-letter opacity-0 scale-90
    w-full max-w-[95vw] md:max-w-2xl mx-auto
    ${isOpened ? 'animate-reveal-active' : ''}
  `}
>
  {/* Floating letter / scroll emerging from chest */}
  <div className="relative w-full bg-gradient-to-b from-[#fff8e1] via-[#fff0c7] to-[#ffe8ad] p-4 md:p-10 rounded-xl shadow-2xl border-4 md:border-8 border-[#d4b35f]/60 transform rotate-0 md:rotate-[-2deg] animate-float-letter overflow-hidden">
    {/* Wax seal / emblem */}
    <div className="absolute -top-6 md:-top-10 left-1/2 -translate-x-1/2 w-12 h-12 md:w-24 md:h-24 bg-[#b8963e] rounded-full border-4 md:border-8 border-amber-300 shadow-lg flex items-center justify-center z-10">
      <Heart size={24} className="text-red-500 md:hidden" fill="currentColor" />
      <Heart size={40} className="text-red-500 hidden md:block" fill="currentColor" />
    </div>

    <div className="text-center space-y-3 md:space-y-6 pt-4 md:pt-8 w-full">
      ...
      <h2 className="text-xl sm:text-2xl md:text-5xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-red-500 via-pink-500 to-purple-500 drop-shadow-[0_0_20px_pink] px-2 leading-tight">
        С Днем Валентина<br className="md:hidden" /> Моя Дорогая<br className="md:hidden" /> Ке Цин! 💞
      </h2>

      <p className="text-sm sm:text-base md:text-2xl text-gray-800 font-serif leading-relaxed px-2 md:px-0" style={{ wordBreak: 'break-word', overflowWrap: 'break-word' }}>
        Я поздравляю тебя с этим прекрасным праздником всех влюбленных, хоть мы и знакомы не так долго, но ты очень приятный человек и мне хочется общаться с тобой и узнавать о тебе больше! Пусть этот день станет особеннее, моя валентинка!
      </p> 

      <div className="flex justify-center gap-3 md:gap-8 mt-4 md:mt-8">
        <Heart size={32} className="text-red-500 animate-pulse md:hidden" fill="currentColor" />
        <Heart size={32} className="text-pink-500 animate-pulse md:hidden" fill="currentColor" style={{ animationDelay: '0.4s' }} />
        <Heart size={32} className="text-purple-500 animate-pulse md:hidden" fill="currentColor" style={{ animationDelay: '0.8s' }} />
